...
本项目聚焦于**调度策略与资源管理的系统化改造**，通过 8 个递进的优化点（其中 3 个已实现），形成一套完整的调度与资源管控方案：
...
- ✅ **已实现（3 项）**：QoS 分级调度、Token 级速率控制、MLFQ 多级反馈队列
- 🔲 **待实现（5 项）**：KV 水位线流控、请求准入控制、Deadline/EDF、WFQ 公平调度、KV Cache 分层存储
