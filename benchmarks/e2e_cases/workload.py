...
def sla_tier_to_priority(sla_tier: str) -> int:
    """Map SLA tier to scheduling priority (lower = higher priority).

    gold   -> -2 (highest priority)
    silver -> 0  (default)
    bronze -> 2  (lowest priority)
    """
    return {"gold": -2, "silver": 0, "bronze": 2}.get(sla_tier, 0)
...
async def send_chat_request(
    ...
    payload = {
        ...
        "temperature": 0.7,
        "stream": True,
        "priority": sla_tier_to_priority(record.sla_tier),
    }
    try:
