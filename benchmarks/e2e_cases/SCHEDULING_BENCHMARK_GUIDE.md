...
### 0.3 TTFT / TPOT / ITL 指标的统计原理（时延全景图）

> **先看这张「时延全景图」**：时延之所以"到处都是"，是因为同一个请求的时延可以在不同位置、用不同切分方式被观察。四个数据源测的是**同一件事的不同角度**，不是重复：

| 数据源 | 时延指标 | 计时位置 | 切分能力 | 回答的问题 | 本项目落点 |
|--------|---------|---------|---------|-----------|-----------|
| **`workload.py`** | TTFT、E2E | 客户端 | **按租户 × 阶段** | 「gold 和 bronze 谁的 TTFT 更慢？」 | §7 公平性结论 |
| **`vllm bench serve`** | TTFT、**TPOT、ITL**、E2E | 客户端 | 全局（单一流量） | 「优化版整体性能退化了吗？」 | §7.4.5 绝对性能 |
| **vLLM `/metrics`** | TTFT、ITL、TPOT、E2E、**queue/prefill/decode 分阶段** | **服务端内部** | 全局 | 「TTFT 慢是卡在排队还是 Prefill？」 | §7.4.6 排队/Prefill |
| **Grafana** | 上面 metrics 的可视化 | （就是 metrics 的展示层） | 全局 | 把 metrics 画成图 | 截图归档 |

**核心认知**：
1. **Grafana 不是新数据源**，它只是 metrics 的展示层。
2. **时延不是某一个工具独有的**——客户端（workload/bench）用秒表测「用户感知时延」，服务端（metrics）分阶段测「引擎内部耗时」，两者差一个网络+框架固定开销。
3. **四者互补、缺一不可**：workload 能按租户切分（metrics/bench 都做不到）；bench 能测 TPOT/ITL/吞吐（workload 没有）；metrics 能拆出排队/Prefill/Decode 分阶段（客户端测不到）。
...
> 本节只保留**最终 A/B 测试结论**。中间的探索过程（根因分析、阈值调优、
> 负载扫描、数据修正、配置排查等）已移到文末「附录 A：优化过程探索记录」，
> 需要了解"做过哪些尝试、踩过哪些坑、结论怎么一步步得出"时再去看。
>
> 测试环境：RTX 5070 Ti（Blackwell，sm_120），Qwen2.5-1.5B-Instruct，
> **无 `--enforce-eager`**（用默认 CUDA graph），单实例，5 阶段混合负载（Phase 1-5，各 60s）。

### 7.0.0 这套优化的价值定位（先明确「不是原创、而是工程系统价值」）

> 在看具体结论前，先厘清这套调度优化的**价值定位**，避免误读为"原创算法"。

**技术原创性 ≠ 工程系统价值，这是两个维度：**

| 维度 | 问的是什么 | 本项目的答案 |
|------|-----------|-------------|
| **技术原创性** | 是否发明了新算法/新理论？ | ❌ 否——QoS/WFQ/MLFQ/EDF/令牌桶/准入控制均为网络、OS、实时系统领域的**成熟经典算法**；其中 QoS 优先级 vLLM 官方已原生内置（`--scheduling-policy priority`） |
| **工程系统价值** | 是否解决了真实系统问题、做出了能跑通并验证清楚的完整系统？ | ✅ 是——这才是本项目的核心价值所在 |

**本项目的工程系统价值体现在五个层面（这是应当主张的贡献）：**

1. **系统性整合**：把 6 个调度优化组织成一条**协同工作、互不冲突**的完整流水线（入口准入 → waiting 排序 → running 控速）。难点在于让多机制正确咬合——例如 Token 限速复用 MLFQ 的 level 分档、MLFQ level 同时喂给 waiting 排序与限速分档、准入控制与 QoS 优先级联动。

2. **真实生产引擎落地**：在高复杂度的 vLLM V1 上实现，趟过大量工程坑——旧版→v0.20.2 的 `RequestQueue` 抽象移植（附录 §A.1）、优先级队列堆不变量、被拒请求拒绝路径完整化（修了 4 个真实 bug，§A.4）、状态跨 waiting↔running 往返维护。

3. **可控实验设计**：每个优化独立开关（`VLLM_DISABLE_*`），支持 A/B 对照与单项消融——可验证、可回归。

4. **诚实的验证与根因分析（含金量最高）**：完整的"实测→证伪→修正"闭环，甚至**推翻了自己早先的错误判断**（§A.6 更正 §A.3 的"GPU 算力耗尽"结论）。最终得出**有边界的诚实结论**：调度优化能保护 silver/bronze（违约率 -53~-56pp），但**救不了 gold（物理算力约束，调度无解）**——讲清边界比只报喜更有价值。

5. **跨领域知识迁移**：把网络（QoS/WFQ/ECN/令牌桶）、OS（MLFQ/EDF）的成熟理念**准确映射**到 LLM 推理场景（如 MLFQ 的"时间片降级"→"token 消耗降级"，令牌桶限速→token_budget 分配）。

**建议的对外表述**（既诚实又凸显价值）：

> 「将网络/OS 领域成熟的调度理念（QoS/WFQ/MLFQ/EDF/令牌桶/准入控制）**系统性地迁移并整合进 vLLM V1 调度器**，形成一条协同工作的完整调度流水线；处理了移植过程中的 API 兼容、堆不变量、拒绝路径等工程难题；并通过一整套 A/B 实验**诚实地验证了优化效果与能力边界**——量化出调度优化能把 silver/bronze 违约率降低 53~56pp，同时明确指出它无法解决物理算力约束（gold 过载）的边界。」

> ⚠️ 避免表述为"原创了 XX 调度算法"；应表述为"**跨领域调度理念的系统性迁移、整合与验证**"，更真实也更有说服力。

### 7.0 汇总：必须的参数配置与测试评估内容（先看这个）

> 本节先给结论——经过整条验证链的实测（详见附录 A），反推出"参数必须怎么配"和"测试评估的是什么"。本节用于快速上手与避坑。

#### 7.0.1 必须的参数配置（踩坑后固化）

以下配置由 §7.2、§7.3 及附录 A 的实测结论反推得出，**任意一项偏离都会导致结果失真或结论错误**：
...
| **`--enforce-eager`** | **必须去掉**（§A.12） | ⚠️ 最大坑：eager 禁用 CUDA graph，prefill 慢 60-100 倍，Phase 3 就 100% 违约，会得出错误结论 |
| `--max-num-seqs`（`max_num_seqs`） | 默认 256，**不要降到 128**（§A.12） | 降到 128 反而加剧排队，256 能容纳更多短请求快速 decode |
...
| 参数 | 必须值 | 原默认值 | 调整理由 |
|------|--------|---------|---------|
| `max_queue_depth` | `40` | 100 | 过高导致准入控制从未触发（§A.2） |
| `overload_violation_threshold` | `0.25` | 0.5 | 更早识别过载 |
| `_sla_violation_window` maxlen | `25` | 50 | 更灵敏反映近期违约率 |
| `_is_high_priority_request` | 仅 `priority < 0`（gold） | 含"短 prompt 即高优" | 旧判定让准入控制形同虚设（§A.4） |
...
| `--bronze-qps-scale` | **必须显式设置**（§A.13、§7.3） | ⚠️ 默认 1.0 时 4800 token × 10 QPS = 48000 tok/s，超硬件 16.7 倍，物理无解；临界过载区间用 `0.2~0.25` 才能测出优化效果 |
| 长文档数据 | **~4800 token**（§A.9 修正后） | 旧数据 230 token 名不副实，掩盖了长文档 Prefill 的真实破坏力 |
...
§A.8 / §7.3 暴露的关键陷阱：**只看违约率会被准入控制欺骗**——拒绝低优请求会让"完成的"请求违约率下降，但这是幸存者偏差。完整评估必须同时看：
...
> 三者结合才能说清楚：优化是"让低优更快"还是"让低优被拒"——本项目结论是后者（§A.8 发现 3）。
...
### 7.3 方案 1 验证：降低 bronze QPS 后 A/B 对比（2026-08-23）
...
2. §A.4/§A.5 准入控制修复 + 根因分析
3. §A.7 隔离实验（长文档是违约唯一根因）
4. §A.8 负载扫描（定位有效边界）
5. §A.9 测试数据修正（230 → 4800 token）
6. §A.10~§A.13 真长文档重测 + 根因排查
7. §7.3 方案 1 验证 + 最终结论

核心结论稳定且可复现：**调度优化（priority + 准入控制）能有效保护 silver/bronze 租户
（-53~-56pp 违约率），但无法保护 gold（最高优先级自身过载，物理无解）。**

### 7.4 指标覆盖度自检：已验证 vs 尚缺（重要）
...
#### 7.4.1 已验证的指标（§7 有实测 A/B 数据）

| 指标 | 来源 | 在 §7 的落点 | 状态 |
|------|------|-------------|------|
| 分阶段 / 分租户**违约率** | `workload.py` | §7.2/§A.4/§A.7/§A.8/§A.10/§7.3 | ✅ 充分 |
| 准入**拒绝次数 / 拒绝率** | `workload.py` + scheduler 日志 | §A.4（6038 次）/§7.3 | ✅ 充分 |
| **完成请求数** | `workload.py` | §A.8/§7.3 | ✅ 有（用于揭示"保护=拒绝"） |
| **P99 TTFT**（零星） | `workload.py` | §7.2（如 94072ms）/§A.7（68ms） | ⚠️ 仅个别点，未系统 A/B |

#### 7.4.2 尚缺的指标（设计了但 §7 未执行 → 已于 §7.4.5 补测）

| 指标 | 应有来源 | 为什么重要 | 当前状态 |
|------|---------|-----------|---------|
| **吞吐（req/s、tok/s）** | `vllm bench serve`（§扩展 0） | reviewer 必问："优化有没有牺牲整体吞吐？"（§扩展 0 原话） | ✅ 已补测（§7.4.5，10 QPS 下零退化） |
| **P99 TPOT** | `vllm bench serve` | Token 限速是否过度拖慢 Decode | ✅ 已补测（§7.4.5，±0%） |
| **P99 ITL** | `vllm bench serve` | 抢占是否影响生成流畅度 | ✅ 已补测（§7.4.5，-1.0%） |
| **P99 E2EL**（系统性） | `vllm bench serve` | 端到端总延迟的 A/B 对比 | ✅ 已补测（§7.4.5，+0.3%） |
| 服务端**排队时间 / Prefill-Decode 耗时** | Prometheus+Grafana（§扩展 -1） | 坐实瓶颈在排队还是 Prefill | ✅ 数字已补（§7.4.6：A 排队 12.2s vs B 0.11s）；截图待补 |

#### 7.4.3 缺口的影响

1. **"保护 = 拒绝" 的代价未被量化**：§7.3 已证明优化靠**拒绝大量 silver/bronze** 来降违约率
...
#### 7.4.5 补测结果：绝对性能 A/B 对比（2026-08-26，无 eager）

> 按 §7.4.4 命令补测，补齐"吞吐 / TPOT / ITL / E2EL"这条腿。两轮均用**无 eager** 配置
> （§A.12 已证明 `--enforce-eager` 是性能大坑，此处对齐生产配置）。
...
结合 §7.3（公平性）+ §7.4.5（绝对性能），调度优化验证的证据链**两条腿齐全**：

1. **公平性腿**（§7.3）：priority 优化保护 silver/bronze，违约率 -53~-56pp
2. **绝对性能腿**（§7.4.5）：正常负载零退化（±5% 内）；过载下用"拒绝"换"低延迟"

#### 7.4.6 补测结果：服务端排队时间 / Prefill 耗时（2026-08-26，无 eager）

> 补齐 §7.4.2 最后一环：服务端内部指标（`vllm:request_queue_time_seconds` /
...
3. **Prefill 不是瓶颈**（此负载下）：两轮 Prefill 都 < 0.5 秒。这修正了 §A.7 之前
...
> 时间窗 20:45 ~ 20:54，**两轮压测在同一窗口内紧邻显示，对比直观**：

![Grafana 调度监控 Dashboard](assets/grafana_scheduling_dashboard.png)

**截图时段标注**（实测时间，按压测实际起止时间）：

| 时间段 | 对应轮次 | 时长 | 结果 |
|--------|---------|------|------|
| **20:49:36 ~ 20:50:16**（左段峰值） | **A 基线（fcfs）** | ~40s | 500 成功 / 0 失败，P99 TTFT = 28.6s |
| **20:51:10 ~ 20:51:22**（右段峰值） | **B 优化（priority）** | ~12s | 67 成功 / **433 失败（86.6%）**，P99 TTFT = 2.8s |

**面板解读（对应 §7.4.6 的数字结论）**：

| 面板 | A 轮（20:49）观察 | B 轮（20:51）观察 | 印证结论 |
|------|-----------------|-----------------|---------|
| 1. 队列长度 | waiting 堆积 ~230，持续时间长 | waiting 也堆积 ~230，但**快速回落**（拒绝清空队列） | B 优化的拒绝机制让队列快速收敛 |
| 4. TTFT P50/P95/P99 | P99 飙到 **40+ 秒** | P99 同样飙到 **30+ 秒**（但只统计幸存请求） | A 端到端延迟严重超标 |
| 5. 请求排队时间 P99 | P99 ~**30 秒** | P99 ~**15 秒**（A 的**一半**）| 准入控制让幸存请求排队时间减半 |
| 6. Prefill vs Decode P95 | Prefill 几乎 0（500 个全部 le<0.5） | 出现 **~7500 异常点**（被拒请求的 prefill 计数） | Prefill 非瓶颈；被拒请求计数异常 |

> **关键对比（面板 5）**：A 基线 P99 排队 30s，B 优化 P99 排队 15s——**B 的幸存请求排队时间比 A 少一半**，
> 印证了"准入控制让幸存者更快"的结论。
>
> 注：面板 4 的 P99（30+ 秒）远高于表格里的「平均排队 12.2s / 0.11s」，因为 P99 是
> 最慢 1% 请求的分位值。精确的均值对比以 §7.4.6 的 histogram 数据为准。
...
> **为什么优化版 Phase 4 仍违约**：在 nsys 时间线上，你会看到 Phase 4 开始后 GPU 利用率立刻顶到 100% 并持续到结束——这直观印证 §A.1 的根因：**不是调度排序问题，而是物理算力打满**。调度优化（QoS/MLFQ 等）只能决定「谁先上」，不能凭空增加算力。
...
> 本节归档 §7 最终结论背后的探索过程（根因分析、阈值调优、负载扫描、数据修正、配置排查等）。
> 正常阅读只需看 §7 的最终结论；需要了解「做过哪些尝试、踩过哪些坑」时再看本节。
...
- [x] 调低准入控制阈值（`max_queue_depth`、`overload_violation_threshold`、窗口大小）
      → 见 §A.4，实测准入控制已触发（6038 次拒绝），但未能降低 gold 违约率
- [x] 验证 priority 优先级在 Phase 4 是否真正生效（gold 应比 bronze 违约率更低）
      → 见 §A.4，gold 确实未被拒绝，但自身高 QPS 导致违约率反而升高
...
### A.4 C 轮测试：准入控制阈值调优（2026-08-22）

> 针对 §A.2 的方案调整，实施了准入控制修复 + 阈值收紧，并重跑优化版（B 轮配置）。
...
但 **gold 违约率不降反升（89.9% → 98.2%）**。原因见 §A.5。

### A.5 根因再分析：为什么准入控制没能保护 gold
...
这个结果**印证了 §A.1 的根因分析**：Phase 4 违约的根本原因是**长文档 Prefill 的绝对耗时 +
...
| **更激进的准入 + 负载分级** | 拒绝时**连队列里已排队的低优请求也驱逐** | ✅ 调度层可做（见 §A.6） |

### A.6 下一步方向：队列驱逐（比准入控制更彻底）
...
> ⚠️ **重要更正（见 §A.7）**：上述"gold 自身 32 QPS 超过 GPU 算力"的判断是**错误的**。
> §A.7 的隔离实验证明 GPU 算力完全够用，真正瓶颈是 **Bronze 长文档 Prefill 的干扰**。

### A.7 隔离实验：长文档是违约的唯一根因（2026-08-23）
...
3. **§A.1 / §A.5 的"GPU 物理算力耗尽"结论是错误的**，需要更正。
...
真正生效**——这是下一步需要排查的方向（见 §A.8）。

### A.8 负载扫描：定位优化方案的有效边界（2026-08-23）
...
### A.9 重要更正：Bronze「长文档」此前名不副实（2026-08-23）
...
2. **"Prefill 预算隔离没生效"的根因**：`is_long_prefill = num_new_tokens > 1024`，
   ...
   而是测试数据没达到阈值。
3. **之前扫描的"长文档负载"实为"长输出负载"**：§A.7 / §A.8 的隔离实验与负载扫描，
...
> ⚠️ **因此 §A.7 / §A.8 的扫描结论需要在新数据上重新验证**：旧数据对应的负载是
> 「长输出」，新数据才是真正的「长文档 Prefill」。两者对调度器的压力模型不同，
> 拐点位置与优化方案的相对表现都可能变化，需重新扫描确认（见 §A.10）。

### A.10 重新扫描：真正的长文档 Prefill（2026-08-23）
...
> **重要印证**：这终于证实了 §A.7 隔离实验的原始假设——「长文档 Prefill」确实是
...
### A.11 排查结论：并发 partial prefill 功能缺失 + 长文档 continuation 独占 GPU（2026-08-23）
...
### A.12 验证实验：去掉 enforce-eager 与「让位」逻辑（2026-08-23）
...
### A.13 核心发现：Phase 4 负载远超硬件能力（2026-08-23）
