...
| 服务端**排队时间 / Prefill-Decode 耗时** | Prometheus+Grafana（§扩展 -1） | 坐实瓶颈在排队还是 Prefill | ⚠️ 仍缺，仅 §8 定性提及，无 A/B 截图归档 |
...
| P99 TTFT (ms) | 28654.15 | 2852.19 | B 的"幸存"请求快 10 倍 |
| P99 E2EL (ms) | 30442.38 | 3935.29 | B 的"幸存"请求快 7.7 倍 |
...
> 仍缺的一环（§7.17.2 表格保留 ⚠️）：服务端**排队时间 / Prefill-Decode 耗时**的
> Prometheus+Grafana A/B 截图归档，用于坐实"瓶颈在排队还是 Prefill"。
