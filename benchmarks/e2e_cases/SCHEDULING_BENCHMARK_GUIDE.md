...
> ⚠️ **待补充**：Grafana 面板截图（排队长度 `num_requests_waiting`、排队时间
> `queue_time` P99、Prefill 耗时面板的 A/B 对比图）待补充归档。当前 Grafana 密码
> 已非默认 `admin/admin`，需重新获取访问凭据后补图。数字结论已由上述 metrics
> histogram 数据坐实，截图仅为可视化佐证。
